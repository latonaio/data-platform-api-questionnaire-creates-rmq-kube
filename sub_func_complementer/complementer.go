...
func (c *SubFuncComplementer) ComplementHeader(input *dpfm_api_input_reader.SDC, subfuncSDC *SDC, l *logger.Logger) error {
	s := &SDC{}
	numRange, err := c.ComplementQuestionnaire(input, l)
	if err != nil {
		return xerrors.Errorf("complement questionnaire error: %w", err)
	}
...
func (c *SubFuncComplementer) ComplementQuestionnaire(input *dpfm_api_input_reader.SDC, l *logger.Logger) (*NumberRange, error) {
	rows, err := c.db.Query(
		...
		WHERE (ServiceLabel, FieldNameWithNumberRange) = ( (?, 'Questionnaire') );`, input.ServiceLabel,
	)
	if err != nil {
	...
	nr.LatestNumber++
	input.Header.Questionnaire = &nr.LatestNumber
	return &nr, nil
}
